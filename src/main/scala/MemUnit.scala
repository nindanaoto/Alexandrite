...
class MemUnit(implicit val conf:CAHPConfig) extends Module {
  ...
  io.out.regWrite.writeData := MuxLookup(ld_type, io.ramPort.readData, Seq(LD_LH  -> shifted(15, 0).asSInt, LD_LB  -> shifted(7, 0).asSInt, LD_LHU -> shifted(15, 0).zext, LD_LBU -> shifted(7, 0).zext) )
}